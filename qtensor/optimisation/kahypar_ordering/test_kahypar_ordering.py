...
def get_tw_costs_rgreedy(tn, max_time = 1):
    ...
    rgreedy_tw = int(rgreedy_opt.treewidth)  
...
def get_tw_costs_kahypar(tn):
    ...
    kahypar_tw = int(kahypar_opt.treewidth)   
...
def get_tw_costs_greedy(tn):
    ...
    greedy_tw = int(greedy_opt.treewidth)   
...
def get_tw_costs_tamaki(tn, wait_time, max_tw=64):
    ...
    tamaki_tw = int(tamaki_opt.treewidth)
...
def plot_result(greedy_result, rgreedy_result, kahypar_result, tamaki_result, wait_time_list, fix_num, iter_l, mode):
    ### Plot figure
    f, axs = plt.subplots(1, 2, figsize=(10, 4))
    f.suptitle(f'Fix = {fix_num}, mode = {mode}') #fontsize=16
    plt.sca(axs[0])
    plt.grid()
    plt.plot(iter_l,[math.log2(row[0]) for row in kahypar_result])
    if tamaki_result != None:
        for (count,_) in enumerate(wait_time_list):
            plt.plot(iter_l,[math.log2(row[0]) for row in tamaki_result[count]])
    plt.plot(iter_l,[math.log2(row[0]) for row in greedy_result])
    plt.plot(iter_l,[math.log2(row[0]) for row in rgreedy_result])
    plt.title('Log Order Search Time')
    
    plt.sca(axs[1])
    plt.grid()
    plt.plot(iter_l,[row[1] for row in kahypar_result], label = 'Kahyper') 
    if tamaki_result != None:
        tamaki_result = list(map(list, zip(*tamaki_result))) #important
        for (count,wait_time) in enumerate(wait_time_list):
            plt.plot(iter_l,[row[1] for row in tamaki_result[count]],label = 'Tamaki (%s)' % wait_time)
    plt.plot(iter_l,[row[1] for row in greedy_result], label = 'Greedy')
    plt.plot(iter_l,[row[1] for row in rgreedy_result], label = 'RGreedy')
    plt.title('Width')
    plt.legend()
    ...
    # plt.plot(iter_l,[math.log2(max(x)) for x in mem_temp]) 
    # for (count,_) in enumerate(wait_time_list):
    ...
    # plt.plot(iter_l,[math.log2(sum(x)) for x in flop_temp], label = 'Kahyper')
    # for (count,wait_time) in enumerate(wait_time_list):
    #     flop_temp=[row[3] for row in tamaki_result[count]]
    #     plt.plot(iter_l,[math.log2(max(x)) for x in flop_temp], label = 'Tamaki (%s)' % wait_time)
    # flop_temp=[row[3] for row in greedy_result]
    # plt.plot(iter_l,[math.log2(max(x)) for x in flop_temp], label = 'Greedy')
    # flop_temp=[row[3] for row in rgreedy_result]
    # plt.plot(iter_l,[math.log2(max(x)) for x in flop_temp], label = 'RGreedy')
...
def test_cost_estimation_N(): ### Different N
    ...
    for N in N_list:
        ...
        print('--Width--')
        print(f'Kahypar tw: {kahypar_str[1]}') 
        #for (count,wait_time) in enumerate(wait_time_list):
        ...
        # print(f'Kahypar mem: {math.log2(max(kahypar_str[2]))}') 
        ## for (count,wait_time) in enumerate(wait_time_list):
        ...
        # print(f'Kahypar flops: {math.log2(sum(kahypar_str[3]))}') 
        ## for (count,wait_time) in enumerate(wait_time_list):
    ...
    #plot_result(greedy_result, rgreedy_result, kahypar_result, tamaki_result, wait_time_list, p, N_list, mode)

    
def test_cost_estimation_p(): ### Different p
    #tamaki_result, kahypar_result, greedy_result, rgreedy_result=[],[],[],[]
    mode = 'ansatz'
    p_list = [1,2] #,4,6,8,10
    for p in p_list:
        N = 60
    ...
    #plot_result(greedy_result, rgreedy_result, kahypar_result, tamaki_result, wait_time_list, N, p_list, mode)
...
def test_qtree():
    ...
    p_list = [1,2,3,4]
    mode = 'energy'
    #wait_time = 1; t_tamaki_list = [] 
    t_greedy_list = []; t_kahypar_list = []
    for p in p_list:
        composer, tn = generate_problem(N,p,mode=mode)
        ###
