...
'''
def gen_circ_peo_report(circuit, peo, backend_name, gen_base):
...
def gen_circ_peo_report_mix(circuit, peo, backend_name, gen_base):
...
def collect_reports(circuit, peo, backend_name, repeat, gen_base):
    ...
    for _ in range(repeat):
        if type(backend_name) == list:
            titles, report_table = gen_circ_peo_report_mix(circuit, peo, backend_name, gen_base)
        else:
            titles, report_table = gen_circ_peo_report(circuit, peo, backend_name, gen_base)
...
if __name__ == '__main__':
    backends = ['einsum', 'torch_cpu', 'torch_gpu', 'cupy',["einsum","cupy",16]] #'tr_torch'
    my_algo = 'greedy'
    my_reap = 3
