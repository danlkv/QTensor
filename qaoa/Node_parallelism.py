...
def get_example_task(A=8, B=10, C=7, dim1=0):
    ...
    # bug in range took long to find,(first arg=1)
...
def contract(A, B):
    ...
    if False:
        with print_lock:
            print('\n>>>\t>>>start',t, os.getpid())
            print(
                'st\t',a.strides, b.strides
                ,'\nsh\t', a.shape, b.shape
                ,'\nix\t', idxa, idxb
            )
        time.sleep(.1)
    print(a.strides, b.strides, idxa, idxb, result_idx)
    shapes = {i:s for i,s in zip(idxa+idxb, a.shape+b.shape)}
    target_shape = [shapes[i] for i in result_idx]
    C = np.empty(target_shape)
    np.einsum(a,idxa, b,idxb, result_idx, out=C)
    print('<<<\t<<<end', t, os.getpid())
    e = time.time()
    print('TTT',e-s)
    return C
...
def sliced_contract(x, y, idxs, num):
    ...
    if False:
        with print_lock:
            t = threading.get_ident()
            print(num, slices)
            print('as0',t, x[0].shape)
            print('bs0',t, y[0].shape)
            print('as',t, a.shape)
            print('bs',t, b.shape)
    a = x[0][slices[0]]
    b = y[0][slices[1]]
    ...
    start = time.time()
    #with pyrof.timing(f'\tcontract sliced {num}\t len sliced:{len(idxs)}'):
    C = contract((a, x[1]), (b, y[1]))
    end = time.time()
    dur = end-start
    N = len(idxs)
    print( f'\tcontract sliced {num}\t len sliced:{N}\t T={dur}\tT*2^n={dur*2**N}', )
    return C
...
x,y = get_example_task(A=20, B=9, C=8, dim1=1)
print('xs', x[0].shape)
print('ys', y[0].shape)
contract_idx = set(x[1]) & set(y[1])
result_idx = set(x[1] + y[1])
...
print('start')
try:
    skip_right_to_n = int(sys.argv[1])
    ranks = [skip_right_to_n]
except:
    print(sys.argv)
    ranks=range(1,7)
...
for rank in ranks: 
    ...
    with prof_thread.timing('Multithread: total'):
        ...
        with prof_thread.timing('Multithread: work'):
            print('range', threads,range(threads))
